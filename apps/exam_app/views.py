...
def dashboard(request):
    if 'id' in request.session:
        context = {
        'mytrips':Trip.objects.filter(joiners__id = request.session['id']),
        'othertrips':Trip.objects.exclude(planner = request.session['id']) & Trip.objects.exclude(joiners__id = request.session['id']) ,
        'mycreatedtrips':Trip.objects.filter(planner_id=request.session['id'])
        }
...
def remove(request, trip_id):
    ...
    Trip.objects.get(id=trip_id).joiners.remove(User.objects.get(id=request.session['id']))
